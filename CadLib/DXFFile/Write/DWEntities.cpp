 ...
 *----------------------------------------------------------------------*/
/*
BOOL dxfPolyLine( HDXF hDxf, PDXFENTVERTEX pVertex,PDXFENTVERTEX pFitPoints, int nVertex, double sWidth,double eWidth,double Elevation, unsigned short Flag )
{
	PDXF	pDxf;
	BOOL	result;
  ...
  // Width
  result &= dxfWriteParamDouble(hDxf, 40, sWidth);
  result &= dxfWriteParamDouble(hDxf, 41, eWidth);
  // Elevation
  result &= dxfWriteParamDouble(hDxf, 30, Elevation);
...
//	result &= dxfWriteParamString(hDxf, 30, "0.0");   /// Carlos

	// VERTEX
	for(int ipoint=0; ipoint<nVertex; ipoint++)
  {
    if(Flag != 4 || ipoint == 2)       //--- Não está funcionando para spline (aho que esta precisando de mais fit-points) por enquanto flag nunca vem com 4
    {
...
//		result &= dxfWriteParamDouble(hDxf, 30, 0);
      ...
      if  (Flag == 4 && ipoint == 2)  
       result &= dxfWriteParamInteger(hDxf,70, 8);  //fit-point
    }

    if(Flag == 4)    //--- Desenha os outros fit-points
    {
      for (int iFitPoint = 0 ; iFitPoint < 4 ; iFitPoint++ )
      {
        PDXFENTVERTEX p = pFitPoints + iFitPoint;

        result &= dxfWriteParamString(hDxf, 0, "VERTEX");
		    // Layer Name
		    result &= dxfWriteParamString(hDxf, 8, pDxf->Write.CurrentLayer);
		    // x
		    result &= dxfWriteParamDouble(hDxf, 10,p->Point.x);
		    // y
		    result &= dxfWriteParamDouble(hDxf, 20, p->Point.y);
		    // z
//		  result &= dxfWriteParamDouble(hDxf, 30, 0);
       result &= dxfWriteParamInteger(hDxf,70, 8);  //fit-point
      }
  ...
  if(Flag == 4)    //--- Desenha os control points
  {
    for(int ipoint=0; ipoint<nVertex; ipoint++)
    {
       if(ipoint != 2)  
       {
		      ...
		      // z
          //result &= dxfWriteParamDouble(hDxf, 30, 0);
          result &= dxfWriteParamInteger(hDxf,70, 16);    // Control points - (Vertices)
       }
	  }	
  }
...
/*----------------------------------------------------------------------*
 ...
 |  Output: TRUE if everything is ok                                    |
 *----------------------------------------------------------------------*/
BOOL dxfPolyLine( HDXF hDxf, PDXFENTVERTEX pVertex, int nVertex, double sWidth,double eWidth,double Elevation, unsigned short Flag )
{
	PDXF	pDxf;
	BOOL	result;
...
//	result &= dxfWriteParamString(hDxf, 30, "0.0");   /// Carlos
  ...
  for(int ipoint=0; ipoint < nVertex; ipoint++)
  {
    result &= dxfWriteParamString(hDxf, 0, "VERTEX");
		// Layer Name
		result &= dxfWriteParamString(hDxf, 8, pDxf->Write.CurrentLayer);
    //--- 3d polyline? 3d vertex.
    if (Flag == 8)
    {
      result &= dxfWriteParamInteger(hDxf, 70, 32);
    }
		...
		result &= dxfWriteParamDouble(hDxf, 20, pVertex[ipoint].Point.y);
		// z
    if (Flag == 8) result &= dxfWriteParamDouble(hDxf, 30, pVertex[ipoint].Point.z);
	}	
...
/*-------------------------------------------------------------------*
 ...
 |  Output: TRUE if everything is ok                                 |
 *-------------------------------------------------------------------*/
BOOL dxfLine( HDXF hDxf, double x1, double y1, double x2, double y2,double z1,double z2)
{
	PDXF	pDxf;
	BOOL	result;
  ...
  // z1
  if(z1 != 0.0) result &= dxfWriteParamDouble(hDxf, 30, z1);
	// x2
	result &= dxfWriteParamDouble(hDxf, 11, x2);
	// y2
	result &= dxfWriteParamDouble(hDxf, 21, y2);
  // z2
  if (z2 != 0.0) result &= dxfWriteParamDouble(hDxf, 31, z2);
...
/*-------------------------------------------------------------------*
 ...
 |  Output: TRUE if everything is ok                                 |
 *-------------------------------------------------------------------*/
BOOL dxfCircle( HDXF hDxf, double cx, double cy, double r,double cz)
{
	PDXF	pDxf;
	BOOL	result;
    ...
    // cz 
    result &= dxfWriteParamDouble(hDxf, 30, cz);
	// radius
...
/*-------------------------------------------------------------------*
 ...
 |    DWORD64 Justification = horizontal & vertical text justification |
 |    double WidthFactor = Relative X scale factor--width (def = 1.0)|
 |    LPCTSTR StyleName = text style name, NULL = current style      |
 |  Output: TRUE if everything is ok                                 |
 *-------------------------------------------------------------------*/
BOOL dxfText( HDXF hDxf, LPCTSTR Text, double x1, double y1, double x2, double y2, double Height, double Rotation, DWORD64 Justification, double WidthFactor, LPCTSTR StyleName )
{
	PDXF	pDxf;
	BOOL	result;
...
/*-------------------------------------------------------------------*
 ...
 |  Output: TRUE if everything is ok                                 |
 *-------------------------------------------------------------------*/
BOOL dxfPoint( HDXF hDxf, double x, double y,double z, double thickness )
{
	PDXF	pDxf;
	BOOL	result;
    ...
    // z 
    result &= dxfWriteParamDouble(hDxf, 30, z);
	// Thickness
	if(thickness!=0.0)
...
BOOL dxfDimLinear( HDXF hDxf, double x1, double y1, double x2, double y2, double x3, double y3, double angle, LPCTSTR text )
{
	...
	for(UINT i=0; i<pDxf->Write.DimStylesNumber; i++)
	{
		...
		pFixDimStyle = (PDXFDIMSTYLE)((DWORD64)pDxf + sizeof(DXF) + sizeof(DXFDIMSTYLE)*i);
	...
	// Begin & End Points ----
	dxfPoint(hDxf, x1, y1, 0.0, 0);
	dxfPoint(hDxf, x2, y2, 0.0, 0);
...
BOOL dxfSpLine5P( HDXF hDxf,PDXFENTVERTEX pVertex,double Y1,double Y2)
{
  PDXF	pDxf;
	BOOL	result;

 	// Initialize pDxf ------------------
	if((pDxf = InitilizePDXF(hDxf))==NULL) return FALSE;
  ...
  result &= dxfWriteParamString(hDxf, 0, "SPLINE");
	// Layer Name
 	result &= dxfWriteParamString(hDxf, 8, pDxf->Write.CurrentLayer);
	// Line Color
	if((pDxf->Write.CurrentColor>0) && (pDxf->Write.CurrentColor<256)) 
    ...
    result &= dxfWriteParamString(hDxf, 100, "AcDbSpline");  // Type
    result &= dxfWriteParamDouble(hDxf, 210,0.0);  // Normal vector x
    result &= dxfWriteParamDouble(hDxf, 220,0.0);  // Normal vector y
    result &= dxfWriteParamDouble(hDxf, 230,1.0);  // Normal vector z
    result &= dxfWriteParamInteger(hDxf, 70, 8);   // Flag
    result &= dxfWriteParamInteger(hDxf, 71, 3);   // Degree
    result &= dxfWriteParamInteger(hDxf, 72, 9);   // Knots
    result &= dxfWriteParamInteger(hDxf, 73, 5);   // Control Points
    result &= dxfWriteParamInteger(hDxf, 74, 0);   // Fit Points
    result &= dxfWriteParamDouble(hDxf, 42,1E-6); // Knots Tolerance
    result &= dxfWriteParamDouble(hDxf, 43,1E-6); // Control Point Tolerance
	//result &= dxfWriteParamDouble(hDxf, 44,1E-6); // Fit Point tolerance

	// Knot Values

	for(int i = 0 ; i < 4 ; i++) 	result &= dxfWriteParamDouble(hDxf, 40,0.0);  
	result &= dxfWriteParamDouble(hDxf, 40,Y1);
	for(int i = 0 ; i < 4 ; i++) 	result &= dxfWriteParamDouble(hDxf, 40,Y1+Y2);

	//  fit Points

	for(int i = 0 ; i < 5 ; i++) 
	{
    result &= dxfWriteParamDouble(hDxf, 10,pVertex->Point.x);
		result &= dxfWriteParamDouble(hDxf, 20,pVertex->Point.y);
		result &= dxfWriteParamDouble(hDxf, 30,0.0);

		pVertex++;
	}
//	result &= dxfWriteParamString(hDxf, 8, "0");   // End Entity
	...
	return UnInitilizePDXF(hDxf);
  /*
  result &= dxfWriteParamString(hDxf, 100, "AcDbSpline");
  result &= dxfWriteParamDouble(hDxf,210,0.0);
  result &= dxfWriteParamDouble(hDxf,220,0.0);
  result &= dxfWriteParamDouble(hDxf,230,1.0);
  result &= dxfWriteParamInteger(hDxf,70,Flag);
  result &= dxfWriteParamInteger(hDxf,71,3);
  result &= dxfWriteParamInteger(hDxf,72,nVertex);
  result &= dxfWriteParamInteger(hDxf,73,nVertex / 2);
  result &= dxfWriteParamInteger(hDxf,74,nVertex / 2);
  result &= dxfWriteParamDouble(hDxf,42, 0.000001);
  result &= dxfWriteParamDouble(hDxf,43, 0.000001);
  result &= dxfWriteParamDouble(hDxf,44, 0.000001);
  result &= dxfWriteParamDouble(hDxf,12,pVertex[0].Point.x);
  result &= dxfWriteParamDouble(hDxf,22,pVertex[0].Point.y);
  result &= dxfWriteParamDouble(hDxf,32,0.0);
  result &= dxfWriteParamDouble(hDxf,13,pVertex[0].Point.x);
  result &= dxfWriteParamDouble(hDxf,23,pVertex[0].Point.y);
  result &= dxfWriteParamDouble(hDxf,33,0.0);
...
BOOL dxfImage(HDXF hDxf, REALPOINT pInsertPoint,double pUVect_x,double pUVect_y, int pBrigth, int pClip,char* ppNameArqImage)
{
	PDXF	pDxf;
	BOOL	result;
	...
	result &= dxfWriteParamString(hDxf, 0, "IMAGEDEF");
	result &= dxfWriteParamInteger(hDxf, 5, 253);
	result &= dxfWriteParamInteger(hDxf, 330, 250);
	result &= dxfWriteParamString(hDxf, 100, "AcDbRasterImageDef");
	result &= dxfWriteParamInteger(hDxf, 281, pBrigth);
	result &= dxfWriteParamString(hDxf, 1, ppNameArqImage);
	result &= dxfWriteParamDouble(hDxf, 10, pInsertPoint.x);
	result &= dxfWriteParamDouble(hDxf, 20, pInsertPoint.y);
	result &= dxfWriteParamDouble(hDxf, 11, pUVect_x);
	result &= dxfWriteParamDouble(hDxf, 21, pUVect_y);
	result &= dxfWriteParamInteger(hDxf, 280, pClip);
	result &= dxfWriteParamInteger(hDxf, 281, pBrigth);
