...
/*-------------------------------------------------------------------*
 ...
 |      DWORD64 dwTableType = ID of table type to begin                |
 |  Output: TRUE if everything is ok                                 |
 *-------------------------------------------------------------------*/
BOOL dxfTableTypeBegin( HDXF hDxf, DWORD64 dwTableType )
{
	PDXF	pDxf;
...
BOOL dxfAddDimStyle( HDXF hDxf, PDXFDIMSTYLE pDimStyle )
{
	...
	CopyMemory((LPVOID)((DWORD64)pDxf+pDxf->Size), pDimStyle, sizeof(DXFDIMSTYLE));
