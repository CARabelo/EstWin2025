...
void WriteEntityDataToFile( HDRAWING hDrawing, HDXF hDxf, PENTITYHEADER pEntityHeader, LPVOID pEntityData )
{
	...
	case ENT_POINT:
		dxfPoint( 
            ...
            ((PENTPOINT)pEntityData)->Point0.z,
...
*/
	...
	case ENT_POLYLINE:
		dxfPolyLine(
            ...
            ((PENTPOLYLINE)pEntityData)->sWidth,
            ((PENTPOLYLINE)pEntityData)->eWidth,
            ((PENTPOLYLINE)pEntityData)->Elevation,
	...
	case ENT_DIMENSION:
		...
		dxfSpLine5P(
			hDxf, 
			(PDXFENTVERTEX)((PENTSPLINE5P)pEntityData)->pVertex,
            ((PENTSPLINE5P)pEntityData)->Knot1,
            ((PENTSPLINE5P)pEntityData)->Knot2
        );
		break;
	    case ENT_IMAGE:
		dxfImage(
			hDxf, 
			(REALPOINT)((PENTIMAGE)pEntityData)->InsertPoint,
      ((PENTIMAGE)pEntityData)->UVect_x,
      ((PENTIMAGE)pEntityData)->UVect_y,
			((PENTIMAGE)pEntityData)->Clipping,
			((PENTIMAGE)pEntityData)->Bright,
			((PENTIMAGE)pEntityData)->File
     );
	}
}
...
/*-------------------------------------------------------------------*
 ...
 |  Inputs:                                                          |
 |      HDRAWING hDrawing = handle to the current DRAWING structure  |
 |      DWORD64 Reserved = reserved, must be 0                         |
 ...
 *-------------------------------------------------------------------*/
BOOL drwSaveDataToFile( HDRAWING hDrawing, DWORD64 Reserved, LPCTSTR strFileName, HWND hWndProgress )
{
	PDRAWING	pDrawing;
	HDXF		hDxf;
	...
	DXFLTYPE	ltype;
	for(int i=0; i<pDrawing->Tables.NumLTypes; i++)
	{
	...
	for(int i=0; i<pDrawing->Tables.NumLayers; i++)
	{
		LTYPE	LType;
		char*	pLTypeName;
	...
	DXFSTYLE tstyle;
	for(int i=0; i<pDrawing->Tables.NumStyles; i++)
	{
	...
	for(int i=0; i<pDrawing->Tables.NumDimStyles; i++)
	{
		STYLE	Style;
		char*	pStyleName;
...
*/
			...
			case ENT_POLYLINE:
                ...
                Polyline.sWidth = ((PENTPOLYLINE)&DxfEntityData)->sWidth;
                Polyline.eWidth = ((PENTPOLYLINE)&DxfEntityData)->eWidth;
                Polyline.Elevation =  ((PENTPOLYLINE)&DxfEntityData)->Elevation;
...
/*-------------------------------------------------------------------*
 ...
 |  Output: TRUE if everything is ok                                 |
 *-------------------------------------------------------------------*/
BOOL drwLoadDataFromFile( HDRAWING hDrawing, OBJHANDLE BlockObjhandle, DWORD64 Reserved, LPCTSTR strFileName, HWND hWndProgress )
{
	PDRAWING	pDrawing;
	HDXF		hDxf;
	PDXF		pDxf;

	DWORD64		dwCurrentSection;
	DWORD64		dwCurrentTableType;
