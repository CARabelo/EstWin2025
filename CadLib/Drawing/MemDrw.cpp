...
/*-------------------------------------------------------------------*
 ...
 |      DWORD64 dwTableType = table type to get data                   |
 |                        ( TAB_NOTSET = search in all table types ) |
 |      DWORD64 dwFindType = find type                                 |
 |      LPVOID pTableType = pointer to table type structure          |
 |  Output: Handle of found table type (0=Error or Not found)        |
 *-------------------------------------------------------------------*/
OBJHANDLE drwFindTableType_Direct( PDRAWING pDrawing, DWORD64 dwTableType, DWORD64 dwFindType, LPVOID pTableType )
{
	OBJHANDLE	result=0;
...
/*-------------------------------------------------------------------*
 ...
 |  Output: Handle of found table type (0=Error or Not found)        |
 *-------------------------------------------------------------------*/
OBJHANDLE drwFindTableType( HDRAWING hDrawing, DWORD64 dwTableType, DWORD64 dwFindType, LPVOID pTableType )
{
	PDRAWING	pDrawing;
	OBJHANDLE	result;
...
/*-------------------------------------------------------------------*
 ...
 |  Inputs:                                                          |
 |      HDRAWING hDrawing = handle to the current DRAWING structure  |
 |      DWORD64 dwTableType = table type to be added                   |
 |      LPVOID pTableType = pointer to table type structure to add   |
 |  Output: Handle of new table type (0=Error)                       |
 *-------------------------------------------------------------------*/
OBJHANDLE drwAddTableType( HDRAWING hDrawing, DWORD64 dwTableType, LPVOID pTableType )
{
	PDRAWING	pDrawing;
	int			CurrentTType;
...
/*-------------------------------------------------------------------*
 ...
 |  Inputs:                                                          |
 |      HDRAWING hDrawing = handle to the current DRAWING structure  |
 |      DWORD64 dwTableType = table type to be removed                 |
 |      OBJHANDLE TableTypeObjhandle = handle of table type to remove|
 |  Output: TRUE if everything is ok                                 |
 *-------------------------------------------------------------------*/
BOOL drwDeleteTableType( HDRAWING hDrawing, DWORD64 dwTableType, OBJHANDLE TableTypeObjhandle )
{
	PDRAWING	pDrawing;
	BOOL		result;
	UINT		i, j;
 ...
 *-------------------------------------------------------------------*/
OBJHANDLE drwFindBlock_Direct( PDRAWING pDrawing, DWORD64 dwFindType, PBLOCKHEADER pBlockHeader )
{
	OBJHANDLE		result;
	bool			isBlockFound;
	unsigned int	i;
...
/*-------------------------------------------------------------------*
 ...
 |  Output: Handle of found block (0=Error or Not found)             |
 *-------------------------------------------------------------------*/
OBJHANDLE drwFindBlock( HDRAWING hDrawing, DWORD64 dwFindType, PBLOCKHEADER pBlockHeader )
{
	PDRAWING		pDrawing;
	OBJHANDLE		result;
...
/*		if(!GlobalUnlock(pEntities->hEntities) && (GetLastError()!=NO_ERROR))
	...
	CopyMemory((LPVOID)((DWORD64)pEntitiesData+pEntities->FreePos), pEntityHeader, sizeof(ENTITYHEADER));
	CopyMemory((LPVOID)((DWORD64)pEntitiesData+pEntities->FreePos+sizeof(ENTITYHEADER)), (LPVOID)pEntityData, EntitySize);
...
BOOL AddEntity( PDRAWING pDrawing, PENTITIES pEntities, PENTITYHEADER pEntityHeader, LPVOID pEntityData )
{
  ...
  case ENT_SPLINE5P:
		pVertex = new ENTVERTEX[5];
		memcpy(pVertex, ((PENTSPLINE5P)pEntityData)->pVertex, sizeof(ENTVERTEX)*(5));
		((PENTSPLINE5P)pEntityData)->pVertex = pVertex;
		result = AddToEntitiesList(pEntities, pEntityHeader, pEntityData, sizeof(ENTSPLINE5P));
		break;

  case ENT_IMAGE:
	  result = AddToEntitiesList(pEntities, pEntityHeader, pEntityData, sizeof(ENTIMAGE));
	  break;
...
//	ENTITYHEADER	EntityHeader;
	...
	{ // Search for block name
		DWORD64 i = FindBlockPosByHandle( pDrawing, BlockObjhandle );
		if(i<0)
		{
			// The block cannot be found
			return 0;
 ...
 *-------------------------------------------------------------------*/
DWORD64 GetCurrentEntityData( PENTITIES pEntities, PENTITYHEADER pEntityHeader, LPVOID pEntityData )
{
	LPVOID	pEntitiesData;
	DWORD64	EntityDataSize;
	...
	CopyMemory(pEntityHeader, (LPVOID)((DWORD64)pEntitiesData+pEntities->CurrentEntityPos), sizeof(ENTITYHEADER));

	EntityDataSize = pEntityHeader->NextEntityPos - (pEntities->CurrentEntityPos + sizeof(ENTITYHEADER));
	CopyMemory(pEntityData, (LPVOID)((DWORD64)pEntitiesData+pEntities->CurrentEntityPos+sizeof(ENTITYHEADER)), EntityDataSize);
...
/*-------------------------------------------------------------------*
 ...
 |      PENTITYHEADER pEntityHeader = pointer to entity data header  |
 |      LPVOID pEntityData = pointer to entity data structure        |
 |      DWORD64 dwEntityPos = position of entity to set data on it     |
 |  Output: current entity data size (0 = error occured)             |
 *-------------------------------------------------------------------*/
DWORD64 SetEntityData( PENTITIES pEntities, PENTITYHEADER pEntityHeader, LPVOID pEntityData, DWORD64 dwEntityPos )
{
	LPVOID	pEntitiesData;
	DWORD64	EntityDataSize;
	...
	CopyMemory((LPVOID)((DWORD64)pEntitiesData+dwEntityPos), pEntityHeader, sizeof(ENTITYHEADER));

	EntityDataSize = pEntityHeader->NextEntityPos - (dwEntityPos + sizeof(ENTITYHEADER));
	CopyMemory((LPVOID)((DWORD64)pEntitiesData+dwEntityPos+sizeof(ENTITYHEADER)), pEntityData, EntityDataSize);
...
/*-------------------------------------------------------------------*
 ...
 |      LPVOID pEntityData = pointer to entity data structure        |
 |      DWORD64 dwFindType = find type                                 |
 |      DWORD64* pdwEntityPos = position of found entity               |
 |  Output: Handle of found entity (0=Error or Not found)            |
 *-------------------------------------------------------------------*/
OBJHANDLE drwFindEntity_Direct( PDRAWING pDrawing, LPCTSTR strBlockName, PENTITYHEADER pEntityHeader, LPVOID pEntityData, DWORD64 dwFindType, DWORD64* pdwEntityPos )
{
	PENTITIES		pEntities;
	OBJHANDLE		result;
	...
	{ // Search for block name
		DWORD64 i = FindBlockPosByName( pDrawing, strBlockName );
		if(i<0)
		{
			// The block cannot be found
			return 0;
...
/*-------------------------------------------------------------------*
 ...
 |  Output: Handle of found entity (0=Error or Not found)            |
 *-------------------------------------------------------------------*/
OBJHANDLE drwFindEntity( HDRAWING hDrawing, LPCTSTR strBlockName, PENTITYHEADER pEntityHeader, LPVOID pEntityData, DWORD64 dwFindType )
{
	PDRAWING		pDrawing;
	OBJHANDLE		result;
