...
CDxfFileWrite::CDxfFileWrite()
{
	...
	result &= ((dxfCircle = (BOOL(_cdecl*)(HDXF hDxf, double cx, double cy, double r,double cz))
    ...
    result &= ((dxfPoint = (BOOL(_cdecl*)(HDXF hDxf, double cx, double cy, double cz, double thickness))
        GetProcAddress( hinstCadIO, "dxfPoint" )) != NULL);
	...
	//result &= ((dxfImage = (BOOL(_cdecl*)(HDXF, double, double, double, double))
	//	GetProcAddress(hinstCadIO, "dxfImage")) != NULL);
...
BOOL CDxfFileWrite::Circle( double cx, double cy, double r,double cz)
{
	ISLIBRARYLOADED();
	return dxfCircle( m_hDxf, cx, cy, r, cz);
}

BOOL CDxfFileWrite::Point( double cx, double cy, double cz, double Tick)
{
	ISLIBRARYLOADED();
	return dxfPoint(m_hDxf, cx, cy, cz, Tick);
}
...
OBJHANDLE CDrawing::Circle( double cx, double cy, double r,double cz )
{
	ENTCIRCLE circle;
	circle.Point0.x = cx;
	circle.Point0.y = cy;
...
OBJHANDLE CDrawing::Point( double cx, double cy,double cz,double Thick)
{
	ENTPOINT point;
	point.Point0.x = cx;
	point.Point0.y = cy;
	point.Point0.z = cz;
	m_EntityHeader.EntityType = ENT_POINT;
	return AddEntity(ActiveBlockObjhandle, &m_EntityHeader, &point);
}
...
OBJHANDLE CDrawing::PolyLine( PENTVERTEX pVertex, int nVertex,unsigned short Flag,double sWidth,double eWidth,double Elevation)
{
	ENTPOLYLINE polyline;
	polyline.pVertex = pVertex;
	polyline.nVertex = nVertex;
  polyline.sWidth = sWidth;
  polyline.eWidth = eWidth;
...
OBJHANDLE CDrawing::SpLine5P( PENTVERTEX pVertex,double Y1,double Y2)
{
	ENTSPLINE5P spline5p;
	spline5p.pVertex = pVertex;
	spline5p.Knot1 = Y1;
    spline5p.Knot2 = Y2;
	m_EntityHeader.EntityType = ENT_SPLINE5P;
	return AddEntity(ActiveBlockObjhandle, &m_EntityHeader, &spline5p);
}

OBJHANDLE CDrawing::Image(const char* FileName,double cx,double cy, double px, double py)
{
	ENTIMAGE Image;
	strcpy(Image.File, FileName);
	Image.InsertPoint.x = cx;
	Image.InsertPoint.y = cy;
	Image.UVect_x = px;
	Image.UVect_y = py;
	Image.Bright = 5;
	Image.Clipping = 1;
	Image.Manipulator = 253;
	Image.ID = 250;
	strcpy(&Image.Subclass[0], "AcDbRasterImageDef");
	Image.ClassVersion = 0;
		
	m_EntityHeader.EntityType = ENT_IMAGE;
	return AddEntity(ActiveBlockObjhandle, &m_EntityHeader, &Image);
}
