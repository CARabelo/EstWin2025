...
public:
	...
	BOOL Circle( double cx, double cy, double r,double cz=0.0);
    BOOL Point( double cx, double cy, double cz,double Tick=0.0);
...
private:
	...
	BOOL (*dxfPoint)( HDXF hDxf, double cx, double cy,double cz, double thickness);
...
//	BOOL ChangeBlock( OBJHANDLE BlockObjhandle, PBLOCKHEADER pBlockHeader );
	...
	OBJHANDLE Circle( double cx, double cy, double r,double cz=0.0);
 	OBJHANDLE Point( double cx, double cy, double cz,double Thick);
//	OBJHANDLE Ellipse( double cx, double cy, double ex, double ey, double Ratio, double StartParam=0.0, double EndParam=2*PI );
	...
	OBJHANDLE PolyLine( PENTVERTEX pVertex, int nVertex, unsigned short Flag=0,double sWidth=0.0, double eWidth=0.0, double Elevation=0.0);
  OBJHANDLE SpLine5P( PENTVERTEX pVertex,double knot1,double knot2);
	OBJHANDLE Image(const char* FileName,double Cx,double Cy, double Ux, double Uy);

  /*
   result &= drawing.PolyLine(ppEntVertex,
     itLLCN->size(),
     0,
     Thickness,
     Thickness,
     itLLCN->begin()->z);   //--- Desenha a CN
     */
