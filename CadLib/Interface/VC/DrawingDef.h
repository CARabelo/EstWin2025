...
#define COLOR_GRAY       8
#define COLOR_WHITE_GRAY 9
#define COLOR_WHITE_BLUE 140
...
typedef struct tag_ENTSPLINE5P {
	PENTVERTEX		pVertex;
    double Knot1;
    double Knot2; 
} ENTSPLINE5P, *PENTSPLINE5P;
...
typedef struct tag_ENTIMAGE {
  int       Manipulator; 
	int       ID;
	char      Subclass[30];
	int       ClassVersion;
	char	    File[1024];		// Line Type Name
	REALPOINT	InsertPoint;
	double		UVect_x;
	double		UVect_y;
	int       Clipping;
	int       Bright;
...
typedef struct tag_ENTITYHEADER
{
	...
	DWORD64		PreviousEntityPos;
	DWORD64		NextEntityPos;
...
typedef struct tag_ENTITIES
{
	...
	DWORD64		TotalSize;
	DWORD64		FreePos;
	DWORD64		LastEntityPos;
	DWORD64		CurrentEntityPos;
	DWORD64		EntitiesNumber;
} ENTITIES, *PENTITIES;
