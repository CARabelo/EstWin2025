...
int CIconComboBox::AddIcon(LPCTSTR lpszIconFile)
{
	...
	int nIndex = CComboBoxEx::AddString(lpszIconFile);

	if (nIndex != CB_ERR && nIndex != CB_ERRSPACE)
		SetItemData(nIndex, (DWORD_PTR)hIcon);
...
int CIconComboBox::InsertIcon(int nIndex, LPCTSTR lpszIconFile)
{
	...
	nIndex = CComboBoxEx::InsertString(nIndex, lpszIconFile);
...
{
	int nIndex = CComboBoxEx::FindStringExact(-1, lpszIconFile);
	
	if (nIndex == CB_ERR && CComboBoxEx::GetCount() > 0)
		nIndex = 0; // select the first icon if not found

	return CComboBoxEx::SetCurSel(nIndex);
}
