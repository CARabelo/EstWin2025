...
CImageListBox::CImageListBox(CWnd* pParent): JanelaPai(pParent),m_pImageList(nullptr){}
...
void CImageListBox::GetImageFromList(int nImage, CBitmap* destBitmap)
{
 //First we want to create a temporary image list we can manipulate
 CImageList tmpList;
 tmpList.Create(m_pImageList);
...
void CImageListBox::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	CListBox::OnVScroll(nSBCode, nPos, pScrollBar);

	if (JanelaPai) JanelaPai->SendMessage(VUE_USRMSG_ROLOU_IMAGE_LIST, 0, 1);
}
