...
int CalcSuperficieProjTerrpl(std::string& strNomeProj, CProgressCtrl* Progresso, CStatic* pCSTexto, int pConfigSuper, tylstcponto& LstPontosRemovidos, SSetPontos& SetPontosProblema, double AreaTerrap, tylstcponto& LstFronteira, const std::string& FormatoArqPontos, int GeoRef)
{
  ...
  std::ofstream ArqLog(strNomeProj + "_ProjTerraplanagem.log", std::ofstream::out | std::ofstream::trunc);
  ...
  for (int C = 0; C < 2; C++)     //--- Arquivo de restrições e de bordos
  {
    ...
    {
      continue;               //--- O arquivo de bordos não esta mais sendo lido pois a fronteira não esta atualizada nele
      /*
    ...
    else
    {
      ...
      if (C == 1)   //--- bordos??  o Arquivo de bordos é um pouco diferente
      {
 ...
 // if(0) verificar se depende do tipo de superficie
            ...
            while (ArqPontos.good())
            {
              ...
              if (strMarcasComentarios.find(*strBufferNome.begin()) != std::string::npos)
              {
                ...
                if (strBufferNome.find("Pontos") != std::string::npos)
                {
                  ...
                  //--- Os pontos desabilitados simplesmente não são incluídos na superfície.
                      ...
                      else
                      {
                        ...
                        if (!PontoARemover(Point(x, y), LstPontosRemovidos, AreaTerrap))
                        {
                          ...
                          else
                          {
                            ...
                            if (!VerfPontoDentroTerraplanagem(LstPontosRemovidos, Ponto(y,x,z)))
                            {
   ...
   // LeArqCN_AS(NomeArqSuperf, SetCN);   // Por serem cotas inteiras na superficie de drones ocorrem muitas diferencas por arredondamento e a interpolação das cotas fica muito diferente
    ...
    if (SetCN.size())
    {
      for (ittySetPonSuperf it = SetCN.begin(); it != SetCN.end(); ++it)
      {
        ...
        if (!VerfPontoDentroTerraplanagem(LstPontosRemovidos, Ponto(it->x(), it->y(),0.0)))
        {
  ...
  {
    {
      std::ofstream ArqLogSuperf(strNomeProj + "Superf.log", std::ofstream::out | std::ofstream::trunc);

      ArqLogSuperf << std::right << std::fixed << std::setfill(' ') << std::setprecision(3);
    ...
    */
    ...
    Point PVertice;
    unsigned int ContaPonIns(0), contavert(0);

    CArcSuperficie << AngRotacao << (size_t)cdt.number_of_vertices();

    for (CDT::All_vertices_iterator eit = ++cdt.vertices_begin(); eit != cdt.all_vertices_end(); ++eit)   //--- Insere todos os vertices
    //for (CDT::All_vertices_iterator eit : cdt.finite_vertices())
    //for (CDT::All_vertices_iterator eit = ++cdt.all_vertices_begin(); eit != cdt.all_vertices_end(); ++eit)   //--- Insere todos os vertices
    {
      if (fabs(eit->point().x() - 626372.171) < 0.001)
      {
        int i = 0;
      }
...
{
  //--- Remove do terremo os pontos do terreno que estão entro da terraplanagem
