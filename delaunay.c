...
int main (int argc, char * argv[])
{
  ...
  if (add_constraints) gts_fifo_foreach (edges, (GtsFunc) add_constraint, surface);
  ...
  if (!keep_hull) gts_delaunay_remove_hull (surface);

  if (remove_holes) delaunay_remove_holes (surface);
  ...
  if (conform) 
  {
    guint encroached_number = gts_delaunay_conform (surface,steiner_max,(GtsEncroachFunc) gts_vertex_encroaches_edge,NULL);
    if (encroached_number == 0 && refine) 
    {
      ...
      data[1] = &area;
      unrefined_number = 
	    gts_delaunay_refine (surface,steiner_max,(GtsEncroachFunc) gts_vertex_encroaches_edge,NULL,(GtsKeyFunc) triangle_cost,data);
